<template>
    <span>
        ...
        >
            <template #top-options>
                ...
                <div class="float-left border" style="display: flex">
                    ...
                    <AsesorSelector style="width:180px" v-model="supervisorFilter" placeholder="moviles"></AsesorSelector>
                    <div
                        title="Buscar equipo"
        ...
        <modal-component v-if="equipoDataEdit!=null" @close="equipoDataEdit = null">
            <template #body>
                <moviles-form :source="equipoDataEdit.original" @changes="equipoDataEdit.changes = $event"></moviles-form>
            </template>
            <template #footer>
...
export default {
    ...
    methods: {
        cambioEstado(equipo,state){
            ...
            axios
                .put(`/api/moviles/${equipo.id}`, {estado:state})
                .then(result => {
                    this.loadData();
                })
                .catch(err => {
        ...
        updateEquipo(){
            axios
                .put(`/api/moviles/${this.equipoDataEdit.original.id}`, this.equipoDataEdit.changes)
                .then(result => {
                    this.loadData();
        ...
        eliminarEquipo(equipo){
            ...
            axios.delete('/api/moviles/'+equipo.id).then((result) => {
                this.loadData()
            }).catch((err) => {
        ...
        crearEquipo() {
            axios
                .post(`/api/moviles`, this.newEquipoOBject)
                .then(result => {
                    this.loadData();
        ...
        updateAsesor() {
            axios
                .put("/api/moviles/" + this.asesorEditInfo.row.id, {
        ...
        updateSupervisor() {
            axios
                .put("/api/moviles/" + this.supervisorEditInfo.row.id, {
